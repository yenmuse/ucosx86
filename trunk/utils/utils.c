...
static u32 g_cur_pos = 0x0;

void putch(char ch)
{
	u16 *pvideo = (u16*)0xb8000;

	if(ch == '\n')
		g_cur_pos = g_cur_pos / 80 * 80 + 80; //g_cur_pos += 80;
	else if(ch == '\t')
		g_cur_pos = g_cur_pos / 4 * 4 + 4;
	else if(ch != '\r')
	{
		pvideo[g_cur_pos++] = (0xC << 8) | ch;
	}
}
...
int printk(const char *fmt, ...)
{
  ...
  put_string((const char*)printk_buf);
